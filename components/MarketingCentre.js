...
export default function MarketingCentre({contacts=[],campaigns=[]}){
 const [sync,setSync]=useState(''),[state,setState]=useState('idle'),[form,setForm]=useState(initial),[device,setDevice]=useState('phone'),[audienceMode,setAudienceMode]=useState('all'),[query,setQuery]=useState(''),[selectedIds,setSelectedIds]=useState([]);
 ...
 return <div className="marketingCentre marketingV2">
  <section className="marketingSetup audienceBuilder">
   ...
   <div className="audienceSelected"><div><small>SELECTED</small><b>{selected.length} contacts</b><span>{sendEligible.length} currently eligible for Resend marketing</span></div><button className="miniButton" onClick={syncNow}>Sync eligible subscribers</button></div>
   <details className="audiencePeople" open={audienceMode==='custom'}><summary>View / choose contacts <span>{shown.length} shown</span></summary><div className="audienceToolbar"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search name, email, source or label…"/>{audienceMode==='custom'&&<><button type="button" className="miniButton" onClick={selectShown}>Select all shown</button><button type="button" className="miniButton" onClick={()=>setSelectedIds([])}>Clear</button></>}</div><div className="audienceList">{shown.map(c=><label className="audiencePerson" key={c.id}>{audienceMode==='custom'&&<input type="checkbox" checked={selectedIds.includes(c.id)} onChange={()=>toggle(c.id)}/>}<div><b>{c.full_name}</b><small>{c.email}</small></div>{c.is_previous_customer&&<em>Previous customer</em>}<span className={`permissionPill ${c.marketing_status==='subscribed'&&c.marketing_consent?'yes':'no'}`}>{c.marketing_status==='subscribed'&&c.marketing_consent?'Email permission':(c.marketing_status||'unknown').replaceAll('_',' ')}</span></label>)}</div></details>
