...
export default function SessionLifecycleActions({booking,role}){
 ...
 const [mode,setMode]=useState(null),[reason,setReason]=useState('customer_cancelled'),[note,setNote]=useState(''),[busy,setBusy]=useState(false),[msg,setMsg]=useState('');
 ...
 async function refund(){
  if(!confirm(`Refund £${((Number(booking.amount_pence||0)-Number(booking.refunded_amount_pence||0))/100).toFixed(2)} and cancel this booking?`))return;
  setBusy(true);setMsg('');
  const r=await fetch(`/api/admin/bookings/${booking.id}`,{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({refund:true})});
  const j=await r.json().catch(()=>({}));setBusy(false);if(!r.ok)return setMsg(j.error||'Refund failed.');setMsg('Refund issued ✓');router.refresh();
 }
 async function del(){
 ...
 return <div className="sessionManage">
  ...
  {active&&<div className="sessionManageButtons">
    ...
    {refundable&&<button className="engSecondaryAction warningAction" disabled={busy} onClick={refund}>Refund & cancel</button>}
