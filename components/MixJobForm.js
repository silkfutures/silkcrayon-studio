  ...
  return <section className="adminSection">
    ...
    <form className="mixJobForm" onSubmit={submit}>
      ...
      <div className="mixFormGrid">
        <label className="field"><span>Service</span><select name="serviceType" defaultValue="mix_master"><option value="mix_master">Mix &amp; Master</option><option value="mix_only">Mix only</option><option value="master_only">Master only</option><option value="not_sure">Decide after review</option></select><small>Mix &amp; Master is the standard Silkcrayon service. Change this only when the job is specifically mix-only or master-only.</small></label>
        ...
        <label className="field"><span>Payment status</span><select value={paymentMode} onChange={e=>{setPaymentMode(e.target.value);setPaidAmount('')}}><option value="unpaid">Payment due / not paid yet</option><option value="paid">Paid already</option><option value="partial_paid">Deposit / pay later</option></select><small>{paymentMode==='paid'?'The full quote is recorded as received and the mix goes straight to Ready to start.':paymentMode==='partial_paid'?'Record what has arrived; the payment link requests only the remainder.':'The email payment link requests the full quote.'}</small></label>
        {paymentMode!=='paid'&&<label className="field"><span>Payment due</span><input name="paymentDueDate" type="date"/></label>}
        {paymentMode==='partial_paid'&&<label className="field"><span>Already received (£)</span><input name="paidAmount" type="number" min="0" step="0.01" required value={paidAmount} onChange={e=>setPaidAmount(e.target.value)}/><small>{remaining===null?'Enter the quote first.':`Remaining to request: £${remaining.toFixed(2)}`}</small></label>}
