...
export default function PortalStudioOffer(){
 ...
 return <div className="portalOffer"><div><p className="eyebrow">My Studio offer</p><h3>Keep the momentum — 2 hours for <strong>£90</strong>.</h3><p>Usually £100. Add two studio hours to your balance now and use them on a future session.</p>{msg&&<small>{msg}</small>}</div><button className="button primary" disabled={busy} onClick={buy}>{busy?'Opening…':'Get 2 hours · £90 →'}</button></div>
}
