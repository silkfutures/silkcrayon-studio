...
export default function BookingFlow() {
  ...
  return (
    <form className="bookingPanel" onSubmit={submit} onInput={updateReadiness} onChange={updateReadiness}>
      <div className="bookingSection"><span className="step">01</span><div><h2>Choose your session</h2><div className="optionGrid">{Object.entries(services).filter(([slug])=>slug!=="system-test"||showTest).map(([slug,s])=><button type="button" key={slug} className={`option ${service===slug?"active":""}`} onClick={()=>chooseService(slug)}><b>{s.name}</b><small>{slug==="full-day"?"£450":slug==="system-test"?"£0.30":"£60 / hour"}</small></button>)}<a className="option optionLink relaunchOption" href="/relaunch-offer"><b>2 Hours for £100</b><small>Relaunch offer · save £20 · date later</small><span>→</span></a><a className="option optionLink" href="/buy-hours"><b>Studio Hour Packs</b><small>3–10 hours · better rates · date later</small><span>→</span></a><a className="option optionLink giftOption" href="/gift-studio-time"><b>Gift Studio Time</b><small>Choose 1–8 hours</small><span>→</span></a></div></div></div>
