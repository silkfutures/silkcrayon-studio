...
function dryHireArrivalBlock(){return `<div style="margin:26px 0;padding:20px;border:1px solid #302b35;background:#0b090d"><div style="font-size:11px;letter-spacing:2px;color:#C394FF;font-weight:700">GETTING TO SILKCRAYON</div><p style="color:#fff"><b>The studio entrance is in the lane running through the middle of Portland House.</b></p><p>Do not wait at the main Portland House doors. Enter the lane and wait by the studio entrance. Silkcrayon staff will manage access for your hire.</p><p style="margin-bottom:0"><a href="${gettingHereUrl()}" style="color:#C394FF;font-weight:800">See directions, entrance photos & parking →</a></p></div>`;}
...
export async function sendLoggedNotification({booking,customer,type,subject,html}){
 ...
 try{
  ...
  // A notification-log/schema issue must not make a successfully-created
  // booking look like it failed. Still attempt the email directly.
  console.error('Notification log failed; sending email without log',type,e?.message||e);
...
export function reminderEmail(b,c,{firstTime=false,engineer=null}={}){if(b.service_slug==='dry-hire'){const artist=escapeHtml(c.artist_name||c.full_name);return {subject:`Tomorrow at ${String(b.start_time).slice(0,5)} — your Silkcrayon dry hire`,html:shell('Your dry hire is tomorrow.',`<p>Hi ${artist}, your studio is ready for tomorrow.</p><p><b>${escapeHtml(b.service_name)}</b><br>${prettyDate(b.booking_date)}<br>${String(b.start_time).slice(0,5)}–${String(b.end_time).slice(0,5)}<br>${escapeHtml(studioAddress())}</p><div style="margin:24px 0;padding:18px;border:1px solid #4b3966;background:#0d0914"><b style="color:#fff">REMEMBER</b><p>No Silkcrayon engineer is included. Bring the session files, drives, adapters and anything your engineer needs. Setup, exporting and pack-down all sit inside the booked time.</p><p><b>ID check:</b> ${b.dry_hire_id_required===false?'Not required for this booking':c.dry_hire_id_verified_at?'Verified ✓':'Still required before access. We will resend your secure ID-check link.'}</p></div>${dryHireArrivalBlock()}<p><a href="${siteUrl()}/dry-hire-terms" style="color:#C394FF">Review Dry Hire Terms →</a></p>`,{label:'GETTING HERE',href:gettingHereUrl()})}};
...
export function mixMasterPurchaseEmail(p,c,pricing={}){const artist=escapeHtml(c.artist_name||c.full_name||'there');const turnaround=pricing.studioFinishTurnaround||'within 7 days';const revisions=Number(pricing.studioFinishRevisions??1);return {subject:'Your Studio Finish is booked 🎧',html:shell('Your Studio Finish is booked.',`<p>Hi ${artist}, your Studio Finish is now in the Silkcrayon queue.</p><div style="margin:22px 0;padding:20px;border:1px solid #332b3b;background:#0d0a10"><p style="margin-top:0"><b style="color:#fff">What happens next</b><br>We’ll reopen the track with fresh ears for detailed cleanup, vocal polish, effect refinement, balance and a final release-ready master.</p><p><b style="color:#fff">Turnaround</b><br>${escapeHtml(turnaround)}</p><p><b style="color:#fff">Revisions</b><br>${revisions} revision round${revisions===1?'':'s'} ${revisions===1?'is':'are'} included.</p></div><p>If we need missing files or references, the studio will contact you.</p>`,{label:'OPEN MY STUDIO',href:`${siteUrl()}/account`})}}

export function filesReadyEmail(b,c,{href,expiresAt}={}){
 const artist=escapeHtml(c?.artist_name||c?.full_name||'there');
 const review=reviewUrl()?`<div style="margin:24px 0;padding:18px;border:1px solid #302b35;background:#0b090d"><b style="color:#fff">How was your session?</b><p>If you have a minute, an honest Google review really helps an independent studio.</p><p><a href="${reviewUrl()}" style="color:#C394FF;font-weight:800">Leave an honest Google review →</a></p></div>`:'';
 const expiry=expiresAt?`<p style="font-size:12px;color:#9c94a1"><b style="color:#fff">Download window:</b> uploaded files are available for 30 days. Save your copy before the link expires.</p>`:'';
 return {subject:'Your Silkcrayon files are ready 🎧',html:shell('Your files are ready.',`<p>Hi ${artist}, your files from Silkcrayon are ready.</p><p>Use the secure link below to open your delivery.</p>${expiry}<div style="margin:24px 0;padding:18px;border:1px solid #3d3047;background:#0d0a10"><b style="color:#fff">Ready for the next one?</b><p>Keep the momentum while the work is still fresh.</p><p><a href="${siteUrl()}/booking" style="color:#C394FF;font-weight:800">Book your next session →</a></p></div>${review}`,{label:'OPEN YOUR FILES',href})};
}
