...
function config(){const sid=process.env.TWILIO_ACCOUNT_SID,token=process.env.TWILIO_AUTH_TOKEN,service=process.env.TWILIO_MESSAGING_SERVICE_SID,from=process.env.TWILIO_FROM_NUMBER,senderId=process.env.TWILIO_SENDER_ID||'Silkcrayon';if(!sid||!token||(!service&&!from&&!senderId))return null;return {sid,token,service,from,senderId}}
export async function sendSms({to,body,marketing=false}){
 ...
 // Transactional UK messages use the Silkcrayon alphanumeric Sender ID.
 // Marketing stays on the reply-capable Messaging Service/number so STOP replies still work.
 if(!marketing&&c.senderId)form.set('From',c.senderId);else if(c.service)form.set('MessagingServiceSid',c.service);else form.set('From',c.from);
...
export async function sendLoggedSms({booking,customer,type,body,marketing=false}){
 ...
 if(!logId){const {data,error}=await db.from('notification_log').insert({booking_id:booking.id,customer_id:customer.id,notification_type:type,recipient,subject:body.slice(0,80),status:'queued',channel:'sms'}).select('id').single();if(error)throw error;logId=data.id}
