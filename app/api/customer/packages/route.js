...
export async function POST(req){try{
 ...
 if((used||[]).length)return NextResponse.json({error:'You have already used the 2 hours for £90 relaunch offer.'},{status:409});
 ...
 const session=await stripe.checkout.sessions.create({mode:'payment',invoice_creation:{enabled:true},customer_email:ctx.customer.email,client_reference_id:payment.id,metadata:{studio_payment_id:payment.id,customer_id:ctx.customer.id,payment_kind:'relaunch',hours_credit:String(p.hours),offer_code:promo.code},line_items:[{quantity:1,price_data:{currency:'gbp',unit_amount:p.amount,product_data:{name:'Silkcrayon — 2 Hours for £90 Relaunch Offer',description:`Two prepaid studio hours for ${ctx.customer.artist_name||ctx.customer.full_name}`}}}],success_url:`${base}/relaunch-offer?paid=1&session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${base}/account?package=cancelled`});
