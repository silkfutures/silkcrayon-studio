...
export async function DELETE(request){try{const ctx=await getStaffContext();if(!ctx||ctx.profile.role!=='owner')return NextResponse.json({error:'Owner access required.'},{status:403});const {id}=await request.json();if(!id)return NextResponse.json({error:'Blockout id is required.'},{status:400});const db=getAdminDb(),{data:item,error:readError}=await db.from('blockouts').select('external_calendar_id').eq('id',id).single();if(readError)throw readError;if(item.external_calendar_id)return NextResponse.json({error:'This time is controlled by Apple Calendar. Change the event there, then sync.'},{status:409});const {error}=await db.from('blockouts').delete().eq('id',id);if(error)throw error;return NextResponse.json({ok:true});}catch(e){return NextResponse.json({error:e.message},{status:500})}}
