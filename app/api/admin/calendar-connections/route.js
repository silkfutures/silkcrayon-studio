...
export async function POST(req){try{
 ...
 const url=await verifyAppleCalendarUrl(body.url),db=getAdminDb(),row={name,feed_url_encrypted:encryptCalendarUrl(url),active:true,block_all_day:body.blockAllDay!==false,buffer_before_minutes:Math.min(240,Math.max(0,Number(body.bufferBefore)||0)),buffer_after_minutes:Math.min(240,Math.max(0,Number(body.bufferAfter)||0)),ignored_service_slugs:allowedServices(body.ignoredServiceSlugs)};
