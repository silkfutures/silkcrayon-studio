...
import {sendEmail,packagePurchaseEmail} from '../../../../../../lib/notifications';
...
export async function POST(req,{params}){try{
 ...
 let emailSent=false;if(customer.email){const msg=packagePurchaseEmail({description:`${hours} studio hours · transferred from your previous Silkcrayon voucher`,hours_credit:hours,amount_pence:amountPence},customer),sent=await sendEmail({to:customer.email,...msg});emailSent=!!sent.ok;}
 ...
 return NextResponse.json({ok:true,entryId:entry.id,balance,emailSent});
