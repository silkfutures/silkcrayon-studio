...
export async function POST(req,{params}){
 try{
  ...
  const {data:delivery,error:de}=await db.from('session_deliveries').insert({booking_id:b.id,customer_id:b.customer_id,created_by_user_id:ctx.user.id,delivery_type:externalUrl?'link':'upload',external_url:externalUrl,storage_path:storagePath,file_name:fileName,share_token:token}).select().single();if(de)throw de;
  const href=`${site()}/files/${token}`,msg=filesReadyEmail(b,c,{href});let emailSent=false,smsSent=false;
  if(c?.email){const er=await sendEmail({to:c.email,...msg});emailSent=Boolean(er.ok)}
  if(c?.phone){const sr=await sendSms({to:c.phone,body:`Silkcrayon: your files are ready 🎧 ${href} — ready for the next one? Book again: ${site()}/booking`});smsSent=Boolean(sr.ok)}
