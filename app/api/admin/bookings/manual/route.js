...
export async function POST(req){try{
 ...
 if(['pay_by_bank','card_or_bank'].includes(paymentMode)){
  const {data:payment,error:pe}=await db.from('studio_payments').insert({
   ...
   kind:'session',description:`${booking.service_name} · ${booking.booking_date} ${String(booking.start_time).slice(0,5)}`,
  ...
  const session=await stripe.checkout.sessions.create({
   ...
   line_items:[{quantity:1,price_data:{currency:'gbp',unit_amount:amount,product_data:{name:`Silkcrayon — ${booking.service_name}`,description:`${booking.booking_date} · ${String(booking.start_time).slice(0,5)}–${String(booking.end_time).slice(0,5)} · Cardiff Bay`}}}],
  ...
  if(customer.phone)await sendLoggedSms({booking,customer,type:'manual_booking_payment_link_sms',body:`Silkcrayon payment: £${(amount/100).toFixed(2)} for your ${booking.booking_date} session. Pay securely here: ${paymentUrl}`});
 }
