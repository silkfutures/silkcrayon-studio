...
export default async function FilesReady({params}){
 ...
 return <main className="filesReadyPage"><div className="filesReadyCard"><p className="eyebrow">Silkcrayon Studios</p><h1>{expired?'This delivery has expired.':'Your files are ready.'}</h1><p>Hi {artist}, {expired?'this uploaded delivery has reached the end of its 30-day download window.':'your latest Silkcrayon files are ready to collect.'}</p>{available?<><a className="button primary filesDownload" href={`/files/${token}/download`}>Download your files →</a>{expiry&&<p className="filesExpiry">Available until {expiry}. Download and save your copy before then.</p>}</>:expired?<div className="portalNotice">Need the files again? Contact the studio and we can re-deliver them if they’re still available in the studio archive.</div>:<div className="portalNotice">This delivery link needs refreshing. Contact the studio and we’ll sort it.</div>}<div className="filesNextMove"><small>NEXT MOVE</small><h2>Keep building.</h2><p>Book the next session while the work is still fresh.</p><Link className="button outline" href="/booking">Book another session →</Link></div></div></main>
}
