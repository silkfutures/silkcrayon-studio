...
export default async function Payments({searchParams}){
 ...
 return <main className={eng?'engApp':'engineerApp'}>{eng?<EngineerHeader profile={ctx.profile} eyebrow="Payments"/>:<header className="mobileTop"><div><p className="eyebrow">Payments</p><h1>Get paid.</h1></div><AdminNav profile={ctx.profile}/></header>}
   ...
   <section className="engSection"><PaymentCreateForm customers={customers} defaultCustomerId={sp?.customer||''} defaultAmount={sp?.amount||''} defaultDescription={sp?.description||''} defaultPackage={sp?.hours||sp?.package||''} defaultKind={sp?.kind||''} studioFinishPrice={settings.studioFinishPricePence/100}/></section>
